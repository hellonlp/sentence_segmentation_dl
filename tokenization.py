...
class BasicTokenizer(object):
    ...
    def tokenize(self, text):
        ...
        #    #orig_tokens = ['[EMP]', '总', '之', '，', '伤', '心', '，', '恼','[EMP]', '火']
        #    #orig_tokens = [' ',' ','sorry', '总', '之', '，', '伤', '心', '，', '恼',' ', '火']
        #    print('orig_tokens:',orig_tokens)

        # by cm
        orig_tokens = self._tokenize_chinese_chars_NoEngWord(text)
        # print('orig_tokens:',orig_tokens)
    ...
    def _tokenize_chinese_chars_HasEngWord(self, text):
        """Adds whitespace around any CJK character.
        """
        output = []
        for char in text:
            cp = ord(char)
            if self._is_chinese_char(cp) or char == ' ':
                output.append("|=*=|")
                output.append(char)
                output.append("|=*=|")
            else:
                output.append(char)
        return [l for l in ''.join(output).split('|=*=|') if l != '']

    def _tokenize_chinese_chars_NoEngWord(self, text):
        """ List of character. """
        return [l for l in text]
...
if __name__ == '__main__':
    ...
    text = '  sorry 总之，伤心，恼 火'  # '  总之，伤心，恼 火'
...
#    #
#    vocab = load_vocab(hp.vocab_file)
#    wordpiece_tokenizer = WordpieceTokenizer(vocab)
#    print(wordpiece_tokenizer.tokenize(text))
